...
refresh-token rotation. GETs retry up to three times on timeout, network error, 5xx,
and 429 (honouring `Retry-After`) with exponential backoff and full jitter inside a
25-second wall-clock budget. Mutations retry the same way inside a 45-second budget
only when the JSON body carries a `clientOperationId` the server dedupes on;
mutations without one stay single-attempt with manual retry. Exhausting the budget
surfaces the ordinary `ApiError`, so the existing error affordances remain the last
resort.
...
| Studio | QA walking preflight | QA opener enters `Checking evidence…` | Exact missing walking-package evidence is shown with a route back to Spatial authoring | Worker repeats the exact-version navigation and R2-object checks authoritatively; privacy review is the operator's own confirmation on the form |
...
| Studio | Register/review capture bundle | The manifest form enters `Registering bundle…`; version, hardware/export metadata, role selectors, numeric scene-registration fields, rights evidence, close, and submit controls disable together with `aria-busy`; review enters its own `Recording review…` state | Exact API error stays inline; selected assets, role assignments, rights, coordinate frame, transform, evidence asset, limitations, and operation ID remain available for deliberate retry | Stable operation ID and request hash prevent duplicate D1 rows or R2 manifests; the Worker re-resolves exact verified version assets and hashes, independently hashes the capture-to-scene receipt, and preserves it inside the immutable manifest; a blocked contract cannot be accepted or qualify traversal authoring |
| Studio | Export traversal evidence | The selected release action enters `Exporting…` and disables through the shared single-flight controller | Download or authentication failure stays visible and the action restores for deliberate retry; a response without the digest is labelled unusable for qualification | The operator-scoped export is byte-deterministic, server ordered by signed session sequence, and exposes its complete SHA-256 in the response header, file name, and persistent Studio success notice |
...
| Studio | Generate/download measurement DXF | Generating or downloading control pending; conflicting evidence controls disabled during generation | Inline row error, restored controls, explicit retry | Generation is hash-idempotent; authenticated GET retries up to three times and preserves private delivery |
