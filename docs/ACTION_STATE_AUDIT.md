...
| Studio | Upload new capture | One intake form creates the project, preserves the portable visual export and registered metric geometry on one immutable version, reports each multipart upload, then automatically queues splat, floor-plan, collision, and navigation work | Inline dialog error retains project details and exact local files; the created project and multipart parts remain recoverable, while an unavailable spatial draft is shown as a reviewable processing failure rather than silently skipped | Stable project/upload operation IDs make retries resume the same project/files; automatic floor-plan and navigation jobs use deterministic per-asset idempotency keys |
...
| Studio | Correct/rebuild/export vendor-neutral floor plans | The registered Spark render is the primary authoring surface; scene-raycast room/wall/doorway/connector marks and the rebuild control use the shared single-flight action controller, while raw extraction controls stay collapsed under Advanced diagnostics | API/render/normalisation/rebuild/export errors remain attached to the workspace; failed extraction exposes retry, active extraction exposes cancel, and staged render marks remain available until they are deliberately applied | Stable operation IDs make correction-draft, decision, and export mutations replay-safe; immutable proposal and plan hashes bind approved collision/navigation plus indicative SVG/PDF/DXF to their exact source/revision; objective Recast/Rapier/structural validation auto-accepts only final revision-bound builds |
