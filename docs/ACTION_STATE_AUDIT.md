...
| Studio | Open/copy walkable preview | Controls do not exist until the exact version has approved v7+ collision, JSON report, Detour binary, and navigation artifacts; the initiating control then enters `Preparing preview…` or `Creating link…` and reserves a new tab before the async request | Missing, stale, or unverified visual/navigation derivatives leave the project journey intact at `Navigation required before preview` with a direct route to complete the walking map | Tenant-authorized GET re-verifies every derivative in D1 and R2, then mints short-lived exact-version asset tokens and a complete private manifest; no visual-only URL is produced |
...
| Studio | Approve version | Dialog form disabled; `Approving version…` | Inline dialog error names missing privacy or walking-package evidence | Server re-verifies the exact v7+ collision, JSON report, Detour navmesh, and their R2 objects before either the first approval or an idempotent replay |
| Studio | Publish release | Dialog form disabled; `Publishing release…` | Inline dialog error; form retained | Stable client operation ID; D1 returns the original release on replay; an exact duplicate of the active non-token output returns its existing numeric release revision |
