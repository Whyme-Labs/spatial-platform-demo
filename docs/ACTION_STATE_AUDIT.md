...
| Studio | Open/copy processed preview | Initiating control enters `Preparing preview…` or `Creating link…`; a new tab is reserved before the async request so popup blockers do not discard the operator action | Missing or unverified browser derivatives leave the project journey intact and expose the API error for retry | Tenant-authorized GET mints a short-lived exact-version/exact-asset token; the resulting range URL expires and is never cached as public state |
