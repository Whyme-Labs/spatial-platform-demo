...
| --- | --- | --- |
| 1. First-class stages | The project route model includes overview, process, structure, privacy, walk, publish, measurement, and expert. Stage controls update the URL, `aria-current`, focus, and history; a project opens at its first incomplete mandatory stage. Blocked stages remain inspectable and state the next action. | `src/client/studio.ts` (`ProjectSection`, `activateProjectSection`, `firstIncompleteProjectSection`, `projectJourneyStep`); `e2e/release-authoring.spec.ts` (dedicated workspace, keyboard stage activation, Back/Forward, blocked-state cases) |
| 2. Mandatory work outside Expert | Structure, privacy, and publication are routed workspaces. The walk stage dissolved once it held neither a walk test nor its own viewer: routes, the walking profile, and vertical traversals are structural authoring, and build receipts are raw evidence Expert already owns. The Expert route owns raw capture evidence, manual extraction, raw entity geometry, build receipts, and recovery controls. The old mandatory “Advanced evidence and diagnostics” owner is forbidden by the control audit. | `studio.html` project navigation; `src/client/studio.ts` (`renderSpatial`, `renderPublish`); `scripts/audit-control-wiring.mjs`; first-class-stage E2E |
...
| 4. Task-level structure, walk, and publication | The registered render is the default structure editor; raw plan JSON is Expert-only. Walking profiles use presets, build bounds derive from approved structure, starting positions are selected in the rendered scene. The scene has exactly ONE viewer — the page a recipient opens — reached from the Walk stage's "Open the scene"; Studio no longer embeds a second bare copy of the renderer, which is what made "preview" and "published" read as two different things when they are the same page over different data. A stopped walker is told which authority refused the step (reviewed barrier by name, walking-map clearance, or the edge of the captured floor) in that viewer's own HUD. Walking the scene is a check, not a gate: the in-scene walk-test receipt no longer gates publication, because its only assertion was that the end pose differed from the start pose while the processor already proves enclosure, wall sweeps, corner slides, route replay, and reachability, and the operator already approves the build with a typed note. Publication exposes quality presets and Use current view; accepted transforms are read-only; the measurement disclaimer is generated from the approved grade. | `src/client/studio.ts` (`renderSceneAuthoringWorkspace`, `renderWalkTestCard`, `syncNavigationClearancePreset`, `openNavigationBuildDialog`, `syncReleaseTransformModes`, `syncReleaseQualityPreset`, `setProvisionalReleaseDisclaimer`); `studio.html` expert disclosures; release-authoring E2E and shared-policy unit tests |
| 5. Orphan capability decisions | Portfolio tools are exposed to platform administrators. Templates are selectable and apply behavior-driving workflow policy. Saved views and adapter/delivery filters are reachable. Customer email, capture source, and delivery classification are editable. Delivery classifications select canonical privacy, publication, navigation, measurement, hosting, quality, required-file, structure, and clearance policy. Phone/video and drone values are explicitly grouped as specialist evidence sources rather than primary walkable-scene sources. | `studio.html`; `src/client/studio.ts`; `src/shared/project-policies.ts`; `e2e/ui-quality.spec.ts`; `test/project-policies.spec.ts` |
...
| Privacy and walkability first-class | Pass | direct routes and novice workflow |
| Every governed field has consumer and readback | Pass | 244-field registry audit |
